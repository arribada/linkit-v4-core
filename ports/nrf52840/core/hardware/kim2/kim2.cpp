...
#define KIM2_STATE_EQUAL(x) (m_state == x)
...
KIM2Device::KIM2Device() {
	...
	if (configuration_store) {
		m_current_rconf_mode =
		    cached_to_kineis_mod(configuration_store->read_param<unsigned int>(ParamID::ARGOS_CACHED_MODULATION));
	}
	m_timeout = {};
	m_stopping = false;
	m_cmd_is_ok = false;
...
void KIM2Device::bridge_process_rx() {
	if (m_bridge_active) m_kim2_comm.process_rx();
}
...
void KIM2Device::send(const KineisModulation mode, const KineisPacket &user_payload,
	...
	if (mode == KineisModulation::VLDA4 && !m_vlda4_allowed) {
		DEBUG_ERROR("KIM2Device::send: VLDA4 disabled on this unit — required %d dBm not met", KIM2_VLDA4_REQUIRED_DBM);
		notify(KineisEventDeviceError({}));
		return;
	}
	...
	case KineisModulation::LDK:
		if (payload_length > LDK_MAX_LENGTH_BITS) {
			DEBUG_ERROR("KIM2Device::send: LDK payload too long: %d bits (max %d)", payload_length,
			            LDK_MAX_LENGTH_BITS);
	...
	case KineisModulation::LDA2:
		if (payload_length > LDA2_MAX_LENGTH_BITS) {
			DEBUG_ERROR("KIM2Device::send: LDA2 payload too long: %d bits (max %d)", payload_length,
			            LDA2_MAX_LENGTH_BITS);
			notify(KineisEventDeviceError({}));
			return;
		}
	...
	case KineisModulation::VLDA4:
		if (payload_length > VLDA4_MAX_LENGTH_BITS) {
			DEBUG_ERROR("KIM2Device::send: VLDA4 payload too long: %d bits (max %d)", payload_length,
...
bool KIM2Device::switch_modulation(KineisModulation mode, const std::string &rconf_hex) {
	...
	DEBUG_INFO("KIM2Device::%s: switching %d -> %d", __func__, static_cast<int>(m_current_rconf_mode),
	...
	if (sw_v.mismatch) {
		DEBUG_ERROR(
		    "KIM2Device::%s: RCONF for mode %d physically encodes %s (mode %d) — refusing switch (check provisioning)",
...
void KIM2Device::cache_current_modulation() {
	if (configuration_store) {
		configuration_store->write_param(ParamID::ARGOS_CACHED_MODULATION, kineis_mod_to_cached(m_current_rconf_mode));
	}
}
...
void KIM2Device::read_credentials(unsigned int *dec_id, unsigned int *address, std::string *seckey,
	...
	if (send_AT(AT_GET_RCONF)) {
		...
		if (decoded.valid && decoded.modulation == KIM2_MOD_NAME_VLDA4
		...
		if (decoded.valid && actual.has_value() && actual.value() != m_current_rconf_mode
...
bool KIM2Device::resync_rconf_cache() {
	...
	if (rconf.size() == 32) {
		...
		if (write_and_validate_rconf(rconf, m_current_rconf_mode, &decoded) && decoded.valid) {
			std::optional<KineisModulation> actual = KIM2::mod_from_name(decoded.modulation);
			if (actual.has_value() && !(actual.value() == KineisModulation::VLDA4 && !m_vlda4_allowed)) {
				if (actual.value() != m_current_rconf_mode)
					DEBUG_INFO("KIM2Device::resync_rconf_cache: cache %d -> %d (%s)",
					           static_cast<int>(m_current_rconf_mode), static_cast<int>(actual.value()),
					           decoded.modulation.c_str());
				m_current_rconf_mode = actual.value();
				cache_current_modulation();
				ok = true;
			}
		...
		DEBUG_WARN("KIM2Device::resync_rconf_cache: no RCONF configured (len=%u)", static_cast<unsigned>(rconf.size()));
	}
...
void KIM2Device::state_machine(void) {
	switch (m_state) {
	case KIM2ManagerState::power_off: KIM2_STATE_CALL(power_off); break;
	case KIM2ManagerState::power_on: KIM2_STATE_CALL(power_on); break;
	case KIM2ManagerState::init: KIM2_STATE_CALL(init); break;
	case KIM2ManagerState::idle: KIM2_STATE_CALL(idle); break;
	case KIM2ManagerState::transmit: KIM2_STATE_CALL(transmit); break;
	case KIM2ManagerState::receive: KIM2_STATE_CALL(receive); break;
	case KIM2ManagerState::error: KIM2_STATE_CALL(error); break;
	default: break;
	}
}
...
void KIM2Device::run_state_machine(uint16_t delay_ms) {
	system_scheduler->cancel_task(m_task);
	m_task = system_scheduler->post_task_prio([this]() { state_machine(); }, "KIM2StateMachine",
	...
	if (m_packet_buffer.length()) {
		...
		if (!m_rx_supported) {
			...
			DEBUG_ERROR("KIM2Device::state_idle: RX unavailable — module firmware "
			            ...
			            m_fw_version.c_str(), static_cast<unsigned>(KIM2::KIM2_RX_MIN_FW_YEAR),
...
void KIM2Device::state_transmit_enter() {
	...
	if (m_tx_mode != m_current_rconf_mode) {
		...
		if (realign_v.mismatch) {
			DEBUG_ERROR("KIM2Device::state_transmit_enter: RCONF for mode %u physically encodes %s (mode %d) — "
			            "aborting TX (check RCONF provisioning), caching actual modulation",
	...
	DEBUG_INFO("KIM2Device::state_transmit_enter: TX START — RCONF=[%s] mode=%d, payload=%u bytes",
	           m_kim2_comm.m_rconf_info.c_str(), static_cast<int>(m_tx_mode),
...
void KIM2Device::release_wkup_for_burst() {
	...
	unsigned int rn = 0, period = 0;
	if (m_tx_wkup_lowered || !kim2_blind_active(rn, period)) return;
	...
	DEBUG_INFO("KIM2Device: SAT_EXTWAKEUP LOW - module paces its own burst (retx_nb=%u period=%us)", rn, period);
#endif
}
...
void KIM2Device::resume_wkup_after_burst(bool wait_for_wake, const char *reason) {
	if (!m_tx_wkup_lowered) return;
	m_tx_wkup_lowered = false;
	GPIOPins::set(SAT_EXTWAKEUP);
	if (wait_for_wake) PMU::delay_ms(KIM2_DELAY_WKUP_RESUME_MS);
...
void KIM2Device::state_receive_enter() {
	...
	DEBUG_INFO("KIM2Device::state_receive_enter: reception started (safety window %us)", KIM2_RX_SAFETY_WINDOW_S);
...
void KIM2Device::state_receive() {
	...
	if (m_rx_window_ended) {
		DEBUG_INFO("KIM2Device::state_receive: RX window ended%s", m_rx_stop_requested ? " (stop confirmed)" : "");
		m_rx_window_ended = false;
		m_rx_stop_requested = false;
		m_rx_requested = false;
...
void KIM2Device::perform_rx_stop() {
	...
	for (uint8_t attempt = 1; attempt <= KIM2_RX_STOP_ATTEMPTS; attempt++) {
		...
		DEBUG_WARN("KIM2Device::state_receive: AT+DL=0 %s (attempt %u/%u), retrying",
		           rejected ? "rejected" : "unanswered", static_cast<unsigned int>(attempt),
		           static_cast<unsigned int>(KIM2_RX_STOP_ATTEMPTS));

		if (rejected) PMU::delay_ms(KIM2_RX_STOP_RETRY_GAP_MS);
	}
...
void KIM2Device::check_rx_firmware_support() {
	...
	if (m_rx_supported) {
		DEBUG_INFO("KIM2Device: firmware \"%s\" (build %04u-%02u-%02u) — RX available", m_fw_version.c_str(),
...
void KIM2Device::finish_rx_session() {
	if (!m_rx_active) return;

	m_rx_active = false;
	m_rx_stop_requested = false;

	const uint64_t now = PMU::get_timestamp_ms();
	const unsigned int rx_time = (now > m_rx_start_ms) ? static_cast<unsigned int>(now - m_rx_start_ms) : 0;
...
std::string KIM2Device::load_rconf_for_mode(KineisModulation mode) {
	...
	switch (mode) {
	case KineisModulation::LDK: return configuration_store->read_param<std::string>(ParamID::ARGOS_RADIOCONF_LDK);
	case KineisModulation::VLDA4: return configuration_store->read_param<std::string>(ParamID::ARGOS_RADIOCONF_VLDA4);
	case KineisModulation::LDA2:
	default: return configuration_store->read_param<std::string>(ParamID::ARGOS_RADIOCONF_LDA2);
	}
}
...
bool KIM2Device::write_and_validate_rconf(const std::string &rconf_hex, KineisModulation expected_mode,
	...
	if (decoded.modulation == KIM2_MOD_NAME_VLDA4 && decoded.rf_level_dbm != KIM2_VLDA4_REQUIRED_DBM) {
