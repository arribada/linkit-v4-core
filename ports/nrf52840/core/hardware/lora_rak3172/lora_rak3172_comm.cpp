...
LoRaComm::LoRaComm(unsigned int instance) : NrfUartAsync(instance) {}
...
void LoRaComm::on_rx_line(std::string &line) {
	...
	switch (resp) {
	case RESP_OK: notify<LoRaCommEventRespOk>({}); break;
	case RESP_ERROR:
	case RESP_PARAM_ERROR:
	case RESP_BUSY_ERROR:
	case RESP_NO_NETWORK: notify(LoRaCommEventRespError(resp)); break;
	case RESP_EVT_JOINED: notify<LoRaCommEventJoined>({}); break;
	case RESP_EVT_JOIN_FAILED: notify<LoRaCommEventJoinFailed>({}); break;
...
LoRa::RespType LoRaComm::parse_rx_line_protocol(std::string &line) {
	if (line.empty()) return RESP_UNKNOWN;

	// Check for OK
	if (line == OK_RESP) return RESP_OK;

	// Check for errors
	if (line == ERR_RESP) return RESP_ERROR;
	if (line == PARAM_ERR_RESP) return RESP_PARAM_ERROR;
	if (line == BUSY_ERR_RESP) return RESP_BUSY_ERROR;
	if (line == NO_NET_RESP) return RESP_NO_NETWORK;

	// Check for async events
	if (line.compare(0, EVT_PREFIX.size(), EVT_PREFIX.data(), EVT_PREFIX.size()) == 0) {
		if (line == EVT_JOINED) return RESP_EVT_JOINED;
		if (line == EVT_JOIN_FAILED) return RESP_EVT_JOIN_FAILED;
			...
			if (last_colon != std::string::npos && last_colon > 0) {
				...
				if (prev_colon != std::string::npos) {
					...
					for (size_t i = prev_colon + 1; i < last_colon; i++) {
						if (line[i] >= '0' && line[i] <= '9') m_last_rx_port = m_last_rx_port * 10 + (line[i] - '0');
					}
	...
	if (line.compare(0, 6, "Curren") == 0 ||  // "Current Work Mode: ..."
	    line.compare(0, 11, "RAKwireless") == 0 || line.compare(0, 7, "Welcome") == 0
	    || line.compare(0, 7, "LoRaWAN") == 0 || line.compare(0, 6, "Region") == 0) {
	...
	if (line.size() > 3 && line.compare(0, 3, "AT+") == 0) {
		size_t eq = line.find('=');
		if (eq != std::string::npos && eq + 1 < line.size()) value = line.substr(eq + 1);
	} else if (line.size() > 1 && line[0] == '+') {
		size_t colon = line.find(':');
		if (colon != std::string::npos && colon + 1 < line.size()) {
			size_t v_start = colon + 1;
			while (v_start < line.size() && line[v_start] == ' ')
				v_start++;
