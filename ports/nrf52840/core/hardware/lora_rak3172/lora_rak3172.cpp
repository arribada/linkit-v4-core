...
static inline void lora_park_unused_sat_pins() {
	nrf_gpio_cfg(NRF_GPIO_PIN_MAP(0, 29),  // SAT_INT
	             NRF_GPIO_PIN_DIR_INPUT, NRF_GPIO_PIN_INPUT_DISCONNECT, NRF_GPIO_PIN_NOPULL, NRF_GPIO_PIN_S0S1,
...
#define LORA_STATE_EQUAL(x) (m_state == State::x)
...
void LoRaDevice::bridge_process_rx() {
	if (m_bridge_active) m_lora_comm.process_rx();
}
 ...
 */
void LoRaDevice::send(const KineisModulation mode, const KineisPacket &user_payload,
	...
	if (payload_bytes > max_bytes) {
		DEBUG_ERROR("LoRaDevice::send: payload %u bytes > DR%u limit %u — rejecting", payload_bytes, m_config.dr,
		            max_bytes);
...
void LoRaDevice::react(const LoRaCommEventRespError &err) {
	...
	if (err.error_type == LoRa::RESP_NO_NETWORK) m_joined = false;
	m_is_error = true;
}
...
void LoRaDevice::initiate_timeout(unsigned int timeout_ms) {
	cancel_timeout();
	m_timeout.handle = system_scheduler->post_task_prio([this]() { on_timeout(); }, "LoRaTimeout",
...
void LoRaDevice::start_device() {
	if (m_state == State::standby) {
		...
		for (int attempt = 0; attempt < 3 && !wake_ok; attempt++) {
			if (send_AT(AT_TEST)) wake_ok = true;
		}
		if (wake_ok) {
...
void LoRaDevice::run_state_machine(uint16_t delay_ms) {
	system_scheduler->cancel_task(m_task);
	m_task = system_scheduler->post_task_prio([this]() { state_machine(); }, "LoRaStateMachine",
	                                          Scheduler::DEFAULT_PRIORITY, delay_ms);
}
...
void LoRaDevice::state_power_on() {
	...
	if (m_power_on_retry == 1) m_lora_comm.process_rx();
...
void LoRaDevice::state_configure() {
	...
	case 3:
		...
		if (m_config.deveui.empty()) {
			...
			if (!at_error && !m_lora_comm.m_last_value.empty() && m_lora_comm.m_last_value != "0000000000000000") {
				...
				char buf[17];
#ifdef NRF52840_XXAA
				snprintf(buf, sizeof(buf), "%08lX%08lX", (unsigned long)NRF_FICR->DEVICEID[1],
				         (unsigned long)NRF_FICR->DEVICEID[0]);
#else
				snprintf(buf, sizeof(buf), "%08lX%08lX", (unsigned long)0, (unsigned long)PMU::device_identifier());
#endif
	...
	case 4:
		...
		if (m_config.njm == 1) {
			...
			if (m_config.appeui.size() != 16) {
				DEBUG_ERROR("LoRaDevice: invalid APPEUI length %u (expected 16 hex chars)",
				            static_cast<unsigned>(m_config.appeui.size()));
				at_error = true;
				break;
			}
			at_error = !send_AT(AT_SET_APPEUI, m_config.appeui);
			if (!at_error) DEBUG_INFO("LoRaDevice: APPEUI written to module (%s)", m_config.appeui.c_str());
		}
		break;
	...
	case 5:
		...
		if (m_config.njm == 1) {
			...
			if (m_config.appkey.size() != 32) {
				DEBUG_ERROR("LoRaDevice: invalid APPKEY length %u (expected 32 hex chars)",
				            static_cast<unsigned>(m_config.appkey.size()));
				at_error = true;
				break;
			}
			at_error = !send_AT(AT_SET_APPKEY, m_config.appkey);
			if (!at_error) DEBUG_INFO("LoRaDevice: APPKEY written to module (32 hex chars, not logged)");
		} else if (m_config.njm == 0) {
			if (!m_config.devaddr.empty()) {
				if (m_config.devaddr.size() != 8) {
					DEBUG_ERROR("LoRaDevice: invalid DEVADDR length %u (expected 8 hex chars)",
			...
			if (!at_error && !m_config.nwkskey.empty()) {
				if (m_config.nwkskey.size() != 32) {
					DEBUG_ERROR("LoRaDevice: invalid NWKSKEY length %u (expected 32 hex chars)",
			...
			if (!at_error && !m_config.appskey.empty()) {
				if (m_config.appskey.size() != 32) {
					DEBUG_ERROR("LoRaDevice: invalid APPSKEY length %u (expected 32 hex chars)",
	...
	case 13:
		...
		// commands. Runs once per cold boot, not per standby cycle.
		{
			...
			if (send_AT(AT_GET_LPM)) lpm_val = m_lora_comm.m_last_value;
			if (send_AT(AT_GET_LPMLVL)) lpmlvl_val = m_lora_comm.m_last_value;
			DEBUG_INFO("LoRaDevice: low-power readback: LPM=%s LPMLVL=%s (configured LPM=%u; LPMLVL=1 → Stop1 ~3uA "
			           "wake-on-UART1)",
			           lpm_val.c_str(), lpmlvl_val.c_str(), static_cast<unsigned>(m_config.lp_mode));
		}
		break;
	...
	default: LORA_STATE_CHANGE(configure, error); return;
	}
...
bool LoRaDevice::query_join_state() {
	...
	if (!send_AT(AT_GET_NJS)) return false;
...
void LoRaDevice::state_joining_enter() {
	...
	if (otaa_credentials_missing()) {
		DEBUG_ERROR("LoRaDevice: OTAA join refused — LORA_APPEUI(LRP02)=%s LORA_APPKEY(LRP03)=%s; "
		            ...
		            m_config.appeui.empty() ? "UNSET" : "set", m_config.appkey.empty() ? "UNSET" : "set");
		m_is_error = true;
		return;
	}
	...
	const std::string join_args = "1:0:" + std::to_string(JOIN_INTERVAL_S) + ":" + std::to_string(JOIN_ATTEMPTS);
	...
	DEBUG_INFO("LoRaDevice: join started (%u attempts, %u s apart, window %u s)", JOIN_ATTEMPTS, JOIN_INTERVAL_S,
	           JOIN_WINDOW_MS / 1000);
	initiate_timeout(JOIN_WINDOW_MS);
}
...
void LoRaDevice::load_config_from_store() {
	...
	if (fastloc_mode >= (unsigned int)BaseFastlocMode::DEGRADED_PVT) sensor_bits += 60;  // BITS_FASTLOC_QUALITY
#if ENABLE_ALS_SENSOR
	if (configuration_store->read_param<bool>(ParamID::ALS_SENSOR_ENABLE)) sensor_bits += 17;
#endif
#if ENABLE_PH_SENSOR
	if (configuration_store->read_param<bool>(ParamID::PH_SENSOR_ENABLE)) sensor_bits += 14;
#endif
	...
	if (configuration_store->read_param<bool>(ParamID::SEA_TEMP_SENSOR_ENABLE)) sensor_bits += 14;
#endif
#if ENABLE_THERMISTOR_SENSOR
	if (configuration_store->read_param<bool>(ParamID::THERMISTOR_SENSOR_ENABLE)) sensor_bits += 14;
#endif
	...
	uint8_t min_dr = 0;
	if (max_packet_bytes > 115)
		min_dr = 4;  // DR4 (222 B)
	else if (max_packet_bytes > 51)
		min_dr = 3;  // DR3 (115 B)
	else
		min_dr = 0;  // any DR

	if (m_config.dr < min_dr) {
		DEBUG_WARN("LoRaDevice: max packet %u B requires DR≥%u — bumping LORA_DR %u → %u", max_packet_bytes, min_dr,
		           m_config.dr, min_dr);
		m_config.dr = min_dr;
	}

	DEBUG_INFO("LoRaDevice: config NJM=%u BAND=%u DR=%u ADR=%u TXP=%u CFM=%u FPORT=%u CLASS=%c LP=%u", m_config.njm,
	           m_config.band, m_config.dr, m_config.adr, m_config.txp, m_config.cfm, m_config.fport,
	           m_config.device_class, m_config.lp_mode);
	if (!m_config.deveui.empty()) DEBUG_INFO("LoRaDevice: DEVEUI=%s", m_config.deveui.c_str());
	if (!m_config.appeui.empty()) DEBUG_INFO("LoRaDevice: APPEUI=%s", m_config.appeui.c_str());
}
...
bool LoRaDevice::reload_config_if_changed() {
	...
	bool changed = (old_cfg.deveui != m_config.deveui) || (old_cfg.appeui != m_config.appeui)
	               || (old_cfg.appkey != m_config.appkey) || (old_cfg.devaddr != m_config.devaddr)
	               || (old_cfg.appskey != m_config.appskey) || (old_cfg.nwkskey != m_config.nwkskey)
	               || (old_cfg.njm != m_config.njm) || (old_cfg.band != m_config.band) || (old_cfg.dr != m_config.dr)
	               || (old_cfg.adr != m_config.adr) || (old_cfg.txp != m_config.txp) || (old_cfg.cfm != m_config.cfm)
	               || (old_cfg.fport != m_config.fport) || (old_cfg.lp_mode != m_config.lp_mode)
...
LoRaDevice::LoRaCredentials LoRaDevice::read_lora_credentials() {
	...
	if (send_AT(AT_GET_NJM)) {
		...
		if (v == "0")
			...
			DEBUG_WARN("LoRaDevice::read_lora_credentials: unexpected NJM value '%s' — treating as read failure",
			           v.c_str());
...
bool LoRaDevice::write_credentials_from_config() {
	...
	DEBUG_INFO("write_credentials: njm=%u deveui='%s' (len=%u) devaddr='%s' (len=%u) "
	           ...
	           m_config.njm, m_config.deveui.c_str(), static_cast<unsigned>(m_config.deveui.size()),
	           m_config.devaddr.c_str(), static_cast<unsigned>(m_config.devaddr.size()),
	           static_cast<unsigned>(m_config.appeui.size()), static_cast<unsigned>(m_config.appkey.size()),
	           static_cast<unsigned>(m_config.nwkskey.size()), static_cast<unsigned>(m_config.appskey.size()));
	...
	// state.
	if (njm_changing) {
		DEBUG_INFO("write_credentials: NJM changed (%u → %u), waiting for module reboot", module_njm, m_config.njm);
		PMU::delay_ms(3000);
	...
	if (m_config.njm == 1 && !m_config.deveui.empty() && m_config.deveui != "0000000000000000") {
		if (m_config.deveui.size() != 16) {
			DEBUG_ERROR("write_credentials: invalid DEVEUI length %u", static_cast<unsigned>(m_config.deveui.size()));
			return false;
		}
...
bool LoRaDevice::cw_start(uint32_t freq_hz, uint16_t power_dbm, uint16_t duration_s) {
	...
	std::string params = std::to_string(freq_hz) + "," + std::to_string(power_dbm) + "," + std::to_string(duration_s);
...
bool LoRaDevice::ensure_module_awake(unsigned int timeout_ms) {
	if (m_state == State::standby) {
		...
		bool wake_ok = false;
		for (int attempt = 0; attempt < 3 && !wake_ok; attempt++) {
			if (send_AT(AT_TEST, std::nullopt, 500)) wake_ok = true;
		}
		if (wake_ok) {
			m_state = State::idle;
			return true;
