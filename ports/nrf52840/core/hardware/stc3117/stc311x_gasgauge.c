...
#define STC31xx_SLAVE_ADDRESS            0x70   /* STC31xx 8-bit address byte */
#define STC311x_7BIT_SLAVE_ADDRESS       0xE0   /* STC31xx 7-bit address byte */
...
typedef struct  {
	...
	int Rint;        /* internal battery resistance in milliOhms */
	int CurrentFactor;
	int CRateFactor;   /* SOC Change rate factor */
...
static int STC311x_GetStatusWord16(void)
{
	...
	value &= 0x7fff;   //(MSbit is unused, but used for error detection here)
...
/*******************************************************************************
* Function Name  : STC311x_SetInitialParamAndRun
* Description    :  initialize the STC311x parameters
* Input          : rst: init algo param
* Return         : 0
*******************************************************************************/
static void STC311x_SetInitialParamAndRun(void)
{
	int value;
	int i;
...
static int STC311x_Startup(void)
{
	...
	STC311x_SetInitialParamAndRun();  /* set parameters  */
...
static int STC311x_RestoreFromRam(void)
{
	...
	/* read OCV */
	//ocv=STC31xx_ReadWord16(STC311x_REG_OCV);
	STC31xx_ReadWord16(STC311x_REG_OCV);
...
* Return         : error status (STC31XX_OK, !STC31XX_OK)
*******************************************************************************/
static int STC311x_GetUpdatedBatteryData(STC311x_BattDataTypeDef *batt_data)
{
	...
	/* STC311x status */
	batt_data->STC_Status = res;
	if (batt_data->STC_Status & M_GGVM_MSK)
		batt_data->GG_Mode = VM_MODE;   /* VM active */
	else 
		batt_data->GG_Mode = CC_MODE;   /* CC active */
	...
	/* fill the battery status data */
	{
		...
		batt_data->Voltage = value;  /* result in mV */

		/* temperature */
		value=data[10]; 
		if (value>=0x80) value -= 0x100;  /* convert to signed value */
		batt_data->Temperature = value*10;  /* result in 0.1�C */
		...
		batt_data->CC_adj = value;//in 1/512%  
		value=data[3]; value = (value<<8) + data[1];
		if (value>=0x8000) value -= 0x10000;  /* convert to signed value */
		batt_data->VM_adj = value;//in 1/512%  
		...
		batt_data->OCV = value;  /* result in mV */
#endif
		...
		/* Avg current */
		{
			//Note: For STC3117, Average current is available in both Mixed-mode and Voltage-mode 
			//      (real avg_current in Mixed-mode, and estimated avg_current in Voltage mode)

			value=data[12];   //get REG_AVG_CURRENT 16bit register
			value = (value<<8) + data[11];
			if (value>=0x8000) value -= 0x10000;  /* convert to signed value */

			if (batt_data->Vmode == MIXED_MODE) //mixed mode
			{
				value = conv(value, batt_data->CurrentFactor);
				value = value / 4;  /* divide by 4  */
			}
			else //voltage mode
			{
				value = conv(value, batt_data->CRateFactor);
			}
			batt_data->AvgCurrent = value;  /* result in mA */
		}
	...
	case BATT_FULCHARG:  /* full charge state. wait for actual end of charge current */
		...
		else if ( BattData.AvgCurrent < CHG_END_CURRENT ) 
		{
			if ( BattData.AvgVoltage > BATT_CHG_VOLTAGE )
			{
				...
				STC311x_SetSOC(MAX_HRSOC); /* cap to 100% in device register*/
				STC311x_Reset_Adj();
				BattData.SOC=MAX_SOC;  /* cap to 100% */
			}
...
* Return         : 0 is ok, -1 if STC311x not found or I2C error
* Affect         : global STC311x data and gas gauge variables
*******************************************************************************/
