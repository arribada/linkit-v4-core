...
LPS28DFW::LPS28DFW(unsigned int bus, unsigned char address) : m_bus(bus), m_addr(address) {
...
void LPS28DFW::read(double &temperature, double &pressure) {
	...
	if (lps28dfw_init_set(&m_ctx, LPS28DFW_DRV_RDY) != 0 || lps28dfw_mode_set(&m_ctx, &m_mode) != 0) {
