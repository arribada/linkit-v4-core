...
public:
	...
	bool write_best_effort(std::string str) override;
	std::string read_line() override;
	void set_device_name(const std::string &) override;
#ifdef BENCH_TEST
	bool bench_is_advertising() override;
	int bench_adv_mode() override;
	unsigned int bench_probe_advertising() override;
	void bench_inject_disconnect() override;
#endif
...
private:
	...
	static void static_gatt_evt_handler(nrf_ble_gatt_t *p_gatt, nrf_ble_gatt_evt_t const *p_evt) {
		get_instance().gatt_evt_handler(p_gatt, p_evt);
	};
	static void static_ble_evt_handler(ble_evt_t const *p_ble_evt, void *p_context) {
		get_instance().ble_evt_handler(p_ble_evt, p_context);
	};
	...
	static void static_stm_ota_data_handler(uint16_t conn_handle, ble_stm_ota_t *p_stm_ota,
	                                        ble_stm_ota_event_t *p_evt) {
		get_instance().stm_ota_event_handler(conn_handle, p_stm_ota, p_evt);
	};
	...
	static void static_conn_params_error_handler(uint32_t nrf_error) {
		get_instance().conn_params_error_handler(nrf_error);
	}

	uint8_t m_receive_buffer
	    [4128];  // Maximum payload length is FFF=>4095 bytes, allow 32 bytes for header preamble as well
