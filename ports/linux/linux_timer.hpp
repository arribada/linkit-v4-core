...
public:
	...
	uint64_t get_counter() override { return m_counter_value; }

	void set_counter(uint64_t t) { m_counter_value = t; }
	...
	TimerHandle add_schedule(stdext::inplace_function<void(), INPLACE_FUNCTION_SIZE_TIMER> const &task_func,
		...
		while (iter != m_schedules.end()) {
			if (iter->m_target_counter_value > target_count) break;
			iter++;
		}
	...
	static void timer_thread_func(LinuxTimer *parent) {
		...
		while (parent->m_is_running) {
			...
			for (unsigned int k = 0; k < msec; k++) {
				...
				while (true) {
					...
					bool execute_this_schedule = false;
					{
						...
						if (!parent->m_schedules.size()) break;
					...
					if (execute_this_schedule) {
						...
						if (schedule.m_func) schedule.m_func();
					}
				}
			...
			std::this_thread::sleep_for(std::chrono::milliseconds(
			    1));  // Assumes 1 ms timer tick but it is not guaranteed to wake-up after 1 ms
		}
	}
