...
public:
	...
	void update_salinity(uint32_t elapsed_seconds) {
		...
		if (rand() % 10000 == 0) {
			int zone = rand() % 3;
			if (zone == 0)
				base_salinity = 33.0 + (double)rand() / RAND_MAX * 4.0;
			else if (zone == 1)
				base_salinity = 25.0 + (double)rand() / RAND_MAX * 10.0;
			else
				base_salinity = 5.0 + (double)rand() / RAND_MAX * 20.0;
			salinity_drift = 0.0;
		}
	}
};
	...
	void simulate_timestep(uint32_t step_seconds) {
		...
		if (turtle.is_at_surface()) {
			...
			if (gps.has_fix) {
				...
				results.avg_ttf =
				    (results.avg_ttf * (results.gps_fix_count - 1) + gps.fix_attempt_time) / results.gps_fix_count;
			}
		} else {
...
TEST(TurtleSimulation, LongSimulation7Days) {
	...
	printf("Plongees: %u, Remontees: %u\n", results.dive_count, results.surface_count);
	printf("Profondeur max: %.1f m\n", results.max_depth);
	printf("Temps surface: %.1f h (%.1f%%)\n", results.total_surface_time / 3600.0,
	       100.0 * results.total_surface_time / results.total_time_seconds);
	printf("Temps sous l'eau: %.1f h (%.1f%%)\n", results.total_underwater_time / 3600.0,
	       100.0 * results.total_underwater_time / results.total_time_seconds);
	printf("\n--- GPS ---\n");
...
TEST(TurtleSimulation, SWSWithSalinityVariations) {
	...
	while (results.total_time_seconds < SIMULATION_DURATION) {
		...
		if (results.total_time_seconds % 10 == 0) {
			uint16_t adc = environment.calculate_sws_adc(environment.current_salinity, turtle.current_depth,
			                                             !turtle.is_at_surface());
