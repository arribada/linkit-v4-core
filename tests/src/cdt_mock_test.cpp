...
TEST_GROUP(CDTMock) {
	...
	double calculate(unsigned int samples, int16_t real[2], int16_t imag[2], double GF, double CA, double CB,
	                 double CC) {
		double impedence = 0;
...
TEST(CDTMock, CDTConductivityCalculation) {
	...
	mock().expectOneCall("get_real_imaginary.real").onObject(&m_ad5933).andReturnValue(real[0]);
	mock().expectOneCall("get_real_imaginary.imag").onObject(&m_ad5933).andReturnValue(imag[0]);
	mock().expectOneCall("get_real_imaginary.real").onObject(&m_ad5933).andReturnValue(real[1]);
	mock().expectOneCall("get_real_imaginary.imag").onObject(&m_ad5933).andReturnValue(imag[1]);
