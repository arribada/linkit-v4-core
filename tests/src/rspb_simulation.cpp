...
// =====================================================================
static SimResult run_mission(SimConfig cfg, SimMortalityService &svc, FsLog &log, std::vector<Rec> &recs,
	...
	auto uni = [&](double a, double b) { return std::uniform_real_distribution<double>(a, b)(gen); };
	...
	for (long w = 0; w < R.total_wakes; w++) {
		...
		if (active_na) {
			batt_used_noadapt += cfg.active_session_mah;
			R.active_wakes_noadapt++;
		}
		...
		// --- Bird state model ---
		bool dead_phase;
		if (cfg.death_mode == DeathMode::NONE)
			dead_phase = false;
		else
			dead_phase = (day >= cfg.death_day) && !(cfg.recover_day >= 0 && day >= cfg.recover_day);
		...
		if (alive) {
			...
			body_temp = cfg.ambient_temp_c
			            + (cfg.alive_temp_c - cfg.ambient_temp_c) * std::exp(-dt_since_death_h / cfg.temp_decay_tau_h);
		}
		...
		bool gps_valid;
		if (dead_phase && cfg.death_mode == DeathMode::CANOPY)
			gps_valid = false;
		else
			gps_valid = uni(0, 1) < (alive ? 0.85 : 0.90);
		...
		if (gps_valid) {
			prev_rep_lat = rep_lat;
			prev_rep_lon = rep_lon;
			have_prev_rep = true;
		} else
			days[day].nofix++;
		...
		r.day = day;
		r.alive_model = alive;
		r.gps_valid = gps_valid;
		r.activity = activity;
		r.body_temp = body_temp;
		r.moved_m = moved_m;
		r.gps_speed = gps_speed;
		r.confidence = cur_conf;
		r.consecutive_days = cur_consec;
		r.status = cur_status;
		r.modulo = cur_modulo;
		r.batt_pct = batt_pct;
		recs.push_back(r);

		day_act[day] += activity;
		day_temp[day] += body_temp;
		day_moved[day] += moved_m;
		...
		if (alive) {
			...
			if (cfg.death_mode == DeathMode::NONE || day < cfg.death_day) R.modulo_while_alive = cur_modulo;
		}
		if (cur_status == 2) {
			R.ever_confirmed = true;
			if (R.detection_day < 0) R.detection_day = day;
		}
	...
	for (int d = 0; d < cfg.deploy_days; d++) {
		if (days[d].active_wakes > 0) {
			...
			last_act = days[d].avg_activity;
			last_temp = days[d].avg_temp;
			last_moved = days[d].avg_moved;
			last_conf = days[d].end_confidence;
			last_consec = days[d].end_consecutive;
			last_status = days[d].end_status;
			last_modulo = days[d].end_modulo;
		} else {
			days[d].avg_activity = last_act;
			days[d].avg_temp = last_temp;
			days[d].avg_moved = last_moved;
			days[d].end_confidence = last_conf;
			days[d].end_consecutive = last_consec;
			days[d].end_status = last_status;
			days[d].end_modulo = last_modulo;
		}
	}
...
static void write_csv(const std::vector<Rec> &recs) {
	...
	if (!f.is_open()) {
		printf("  WARN: cannot open rspb_mortality.csv\n");
		return;
	}
	...
	for (const auto &r : recs) {
		f << r.epoch << ',' << r.day << ',' << (r.alive_model ? "ALIVE" : "DEAD") << ',' << (r.gps_valid ? 1 : 0) << ','
		  << std::fixed << std::setprecision(1) << r.activity << ',' << std::setprecision(2) << r.body_temp << ','
		  << std::setprecision(1) << r.moved_m << ',' << r.gps_speed << ',' << r.confidence << ',' << r.consecutive_days
		  << ',' << status_str(r.status) << ',' << r.modulo << ',' << std::setprecision(2) << r.batt_pct << "\r\n";
	}
...
static void write_missionlog(const std::vector<Rec> &recs, const SimConfig &cfg) {
	...
	if (!f.is_open()) {
		printf("  WARN: cannot open rspb_mission.log\n");
		return;
	}
	f << "RSPB bird-tracker mortality mission log\n";
	f << "deployment=" << cfg.deploy_days << "d  death_day=" << cfg.death_day << "  confirm_days=" << cfg.confirm_days
	  << "\n";
	f << "----------------------------------------------------------------------\n";
	for (const auto &r : recs) {
		...
		snprintf(line, sizeof(line),
		         ...
		         ts, r.day, r.alive_model ? "ALIVE" : "DEAD", r.activity, r.body_temp, r.moved_m,
		         r.gps_valid ? "fix" : "nofix", r.confidence, r.consecutive_days, status_str(r.status), r.modulo,
		         r.batt_pct);
		f << line;
	}
...
static void export_devlog(LFSFileSystem *fs, const char *logname) {
	...
	if (!f.is_open()) {
		printf("  WARN: cannot open rspb_mortality_log.csv\n");
		return;
	}
	f << fmt.header();
...
static std::string svg_points(const std::vector<double> &v, double vmin, double vmax, double W, double H) {
	std::ostringstream o;
...
static std::string mini_chart(const std::string &title, const std::vector<double> &series, double vmin, double vmax,
                              const char *color, int death_day, int detection_day, int ndays, const std::string &unit) {
	...
	if (death_day >= 0)
		o << "<line x1=\"" << xat(death_day) << "\" y1=\"4\" x2=\"" << xat(death_day) << "\" y2=\"" << (H + 4)
		  << "\" stroke=\"#ff4757\" stroke-dasharray=\"4 3\" stroke-width=\"1.3\"/>";
	if (detection_day >= 0)
		o << "<line x1=\"" << xat(detection_day) << "\" y1=\"4\" x2=\"" << xat(detection_day) << "\" y2=\"" << (H + 4)
		  << "\" stroke=\"#00ff88\" stroke-dasharray=\"4 3\" stroke-width=\"1.3\"/>";
...
static void write_html(const SimResult &R, const std::vector<DayS> &days) {
	...
	if (!html.is_open()) {
		printf("  WARN: cannot open rspb_mortality_report.html\n");
		return;
	}
...
<div class="container">
  ...
  <div class="sub">)"
	     << time_str << R"( &middot; drives the firmware MortalityService over a )" << cfg.deploy_days
	     << R"(-day deployment &middot; )" << R.total_wakes << R"( TPL5111 wakes, )" << R.active_wakes
	     << R"( active</div>
)";

	if (detected && no_false_positive) {
		html << "  <div class=\"verdict ok\">&#x2705; Mortality CONFIRMED " << latency << " days after death (day "
		     << R.detection_day << ") &mdash; no false positive while alive</div>\n";
	} else if (detected && !no_false_positive) {
		html << "  <div class=\"verdict bad\">&#x26A0; Mortality confirmed, but a false positive occurred while the "
		        "bird was alive</div>\n";
	} else {
	...
	auto card = [&](const char *cls, const std::string &v, const char *l) {
		html << "    <div class=\"card " << cls << "\"><div class=\"v\">" << v << "</div><div class=\"l\">" << l
		     << "</div></div>\n";
	};
	...
	{
		std::ostringstream b;
		b << std::fixed << std::setprecision(0) << R.batt_pct << "%";
...

	{
		...
		html << "  <div class=\"panel\" style=\"flex:1 1 100%\"><h3>Mortality confidence (EMA) &amp; consecutive-day "
		        "counter</h3>";
		...
		html << "<line x1=\"" << xat(cfg.death_day) << "\" y1=\"6\" x2=\"" << xat(cfg.death_day) << "\" y2=\""
		     << (6 + H) << "\" stroke=\"#ff4757\" stroke-dasharray=\"4 3\" stroke-width=\"1.5\"/>";
		if (R.detection_day >= 0)
			html << "<line x1=\"" << xat(R.detection_day) << "\" y1=\"6\" x2=\"" << xat(R.detection_day) << "\" y2=\""
			     << (6 + H) << "\" stroke=\"#00ff88\" stroke-dasharray=\"4 3\" stroke-width=\"1.5\"/>";
	...
	html << mini_chart("AXL activity (avg/day)", act, 0, std::max(act_max, 50.0), "#00d4ff", cfg.death_day,
	                   R.detection_day, nd, "cnt");
	html << mini_chart("Body temperature (avg/day)", temp, 0, std::max(temp_max, 42.0), "#ffb648", cfg.death_day,
	                   R.detection_day, nd,
	                   "\xC2\xB0"
	                   "C");
	html << mini_chart("Displacement (avg/day)", moved, 0, std::max(moved_max, 100.0), "#c39bff", cfg.death_day,
	                   R.detection_day, nd, "m");
	html << mini_chart("Battery remaining", batt, 0, 100, "#00ff88", cfg.death_day, R.detection_day, nd, "%");
	html << "  </div>\n";
	...
	html << "MTP01 MORTALITY_ENABLE=<code>1</code> &middot; "
	        "MTP02 ACTIVITY_THRESH=<code>"
	     << cfg.activity_thresh
	     << "</code> &middot; "
	        "MTP03 TEMP_THRESH=<code>"
	     << std::fixed << std::setprecision(1) << cfg.temp_thresh_c
	     << "&deg;C</code> &middot; "
	        "MTP04 GPS_DIST_THRESH=<code>"
	     << cfg.gps_dist_thresh_m << " m</code><br>";
	html << "MTP05 CONFIRM_DAYS=<code>" << cfg.confirm_days
	     << "</code> &middot; "
	        "MTP06 DUTY_CYCLE_MODULO=<code>"
	     << cfg.duty_modulo
	     << "</code> &middot; "
	        "PWP03 BOOT_COUNTER_MODULO (live)=<code>"
	     << cfg.normal_modulo << "</code><br>";
	html << "Duty-cycle adaptation: modulo while alive=<code>" << R.modulo_while_alive
	     << "</code> &rarr; after CONFIRMED=<code>" << R.final_modulo
	     << "</code> &nbsp;(active wakes: " << R.active_wakes << " with adaptation vs " << R.active_wakes_noadapt
	     << " without)<br>";
	html << "Battery @ end: with adaptation=<code>" << std::setprecision(1) << R.batt_pct
	     << "%</code> vs. without=<code>" << R.batt_pct_noadapt
	     << "%</code> &nbsp;(&#x1F50B; fewer active wakes at the dead-bird modulo)<br>";
	...
	for (const auto &d : days) {
		...
		html << "    <tr class=\"" << rowcls << "\"><td>" << d.day << "</td><td>" << (d.alive_model ? "alive" : "dead")
		     << "</td><td>" << std::fixed << std::setprecision(0) << d.avg_activity << "</td><td>"
		     << std::setprecision(1) << d.avg_temp << "</td><td>" << std::setprecision(0) << d.avg_moved << "</td><td>"
		     << d.nofix << "</td><td>" << d.active_wakes << "</td><td>" << d.end_confidence << "</td><td>"
		     << d.end_consecutive << "</td><td><span class=\"badge " << bcls << "\">" << status_str(d.end_status)
		     << "</span></td><td>" << d.end_modulo << "</td><td>" << std::setprecision(1) << d.batt_pct
		     << "</td></tr>\n";
	}
	html << "    </tbody>\n  </table></div>\n";
...
static SimResult run_scenario(SimConfig cfg, LFSFileSystem *fs, const char *logname,
                              std::vector<Rec> *out_recs = nullptr, std::vector<DayS> *out_days = nullptr) {
	FsLog log(fs, logname, 512 * 1024);
	log.create();
...
TEST(RSPBSimulation, OpenFieldDeath) {
	...
	printf("\n[rspb-sim] OPEN: %ld wakes (%ld active) | death=day%d detection=day%d "
	       ...
	       R.total_wakes, R.active_wakes, cfg.death_day, R.detection_day, status_str(R.final_status),
	       R.final_confidence, R.modulo_while_alive, R.final_modulo, R.batt_pct, R.batt_pct_noadapt, R.devlog_entries);
...
TEST(RSPBSimulation, CanopyDeathNoFix) {
	...
	printf("[rspb-sim] CANOPY: detection=day%d final=%s conf=%u%% max_conf=%u\n", R.detection_day,
	       status_str(R.final_status), R.final_confidence, R.max_conf_overall);
...
TEST(RSPBSimulation, LiveBirdNeverConfirmed) {
	...
	printf("[rspb-sim] LIVE(365d): ever_confirmed=%d max_conf=%u final=%s final_modulo=%u\n", (int)R.ever_confirmed,
	       R.max_conf_overall, status_str(R.final_status), R.final_modulo);
...
TEST(RSPBSimulation, FalseAlarmRecovery) {
	...
	printf("[rspb-sim] RECOVERY: detection=day%d recovered=day%d final=%s final_modulo=%u\n", R.detection_day,
	       R.recovered_day, status_str(R.final_status), R.final_modulo);
...
TEST(RSPBSimulation, DutyModuloCorners) {
	...
	}
	{
		...
		printf("[rspb-sim] DUTY=0: ever_confirmed=%d final_modulo=%u (expect unchanged %u)\n", (int)R.ever_confirmed,
		       R.final_modulo, cfg.normal_modulo);
		CHECK_TEXT(R.ever_confirmed, "DUTY=0: mortality detection should still work");
		CHECK_EQUAL_TEXT((int)cfg.normal_modulo, (int)R.final_modulo,
		                 "DUTY=0: modulo changed despite disabled adaptation");
	}
}
...
TEST(RSPBSimulation, CrossYearDeath) {
	...
	printf("[rspb-sim] CROSSYEAR: death=day%d detection=day%d final=%s conf=%u consec=%u\n", cfg.death_day,
	       R.detection_day, status_str(R.final_status), R.final_confidence, R.final_consecutive);
