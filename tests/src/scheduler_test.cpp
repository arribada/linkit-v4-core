...
TEST_GROUP(Scheduler){ void setup(){ timer->start();
}
...
TEST(Scheduler, SchedulerDeferredMultiTaskSingleShot) {
	...
	scheduler->post_task_prio([]() { fired[0] = true; }, "SchedulerDeferredMultiTaskSingleShot0",
	                          scheduler->DEFAULT_PRIORITY, 5);
	scheduler->post_task_prio([]() { fired[1] = true; }, "SchedulerDeferredMultiTaskSingleShot1",
	                          scheduler->DEFAULT_PRIORITY, 10);
	delay_ms(100);
	scheduler->run();
	CHECK_TRUE(fired[0]);
	CHECK_TRUE(fired[1]);
...
TEST(Scheduler, SchedulerCancelDeferredTaskSingleShot) {
	...
	auto t = []() { fired = true; };
	auto task_handle =
	    scheduler->post_task_prio(t, "SchedulerCancelDeferredTaskSingleShot", scheduler->DEFAULT_PRIORITY, 5);
	scheduler->run();
	scheduler->cancel_task(task_handle);
	CHECK_FALSE(scheduler->is_scheduled(task_handle));
	delay_ms(100);
...
TEST(Scheduler, SchedulerSamePriorityOrdering) {
	...
	scheduler->post_task_prio(
	    ...
	    "SchedulerPriorityOrdering0");
	scheduler->post_task_prio(
	    []() {
		    printf("fired1\n");
		    fired[1] = order++;
	    },
	    "SchedulerPriorityOrdering1");
	scheduler->post_task_prio(
	    []() {
		    printf("fired2\n");
		    fired[2] = order++;
	    },
	    "SchedulerPriorityOrdering2");
	scheduler->post_task_prio(
	    []() {
		    printf("fired3\n");
		    fired[3] = order++;
	    },
	    "SchedulerPriorityOrdering3");
	scheduler->post_task_prio(
	    []() {
		    printf("fired4\n");
		    fired[4] = order++;
	    },
	    "SchedulerPriorityOrdering4");
	scheduler->post_task_prio(
	    []() {
		    printf("fired5\n");
		    fired[5] = order++;
	    },
	    "SchedulerPriorityOrdering5");
	scheduler->post_task_prio(
	    []() {
		    printf("fired6\n");
		    fired[6] = order++;
	    },
	    "SchedulerPriorityOrdering6");
	scheduler->run();
...
TEST(Scheduler, SchedulerSamePriorityOrderingWithDelay) {
	...
	scheduler->post_task_prio(
	    ...
	    "SchedulerPriorityOrdering0", 0, 10);
	scheduler->post_task_prio(
	    []() {
		    printf("fired1\n");
		    fired[1] = order++;
	    },
	    "SchedulerPriorityOrdering1", 0, 10);
	scheduler->post_task_prio(
	    []() {
		    printf("fired2\n");
		    fired[2] = order++;
	    },
	    "SchedulerPriorityOrdering2", 0, 10);
	scheduler->post_task_prio(
	    []() {
		    printf("fired3\n");
		    fired[3] = order++;
	    },
	    "SchedulerPriorityOrdering3", 0, 10);
	scheduler->post_task_prio(
	    []() {
		    printf("fired4\n");
		    fired[4] = order++;
	    },
	    "SchedulerPriorityOrdering4", 0, 10);
	scheduler->post_task_prio(
	    []() {
		    printf("fired5\n");
		    fired[5] = order++;
	    },
	    "SchedulerPriorityOrdering5", 0, 10);
	scheduler->post_task_prio(
	    []() {
		    printf("fired6\n");
		    fired[6] = order++;
	    },
	    "SchedulerPriorityOrdering6", 0, 10);
	delay_ms(20);
	scheduler->run();
	CHECK_EQUAL(0, fired[0]);
	CHECK_EQUAL(1, fired[1]);
