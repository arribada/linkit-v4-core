#include "CppUTest/TestHarness.h"
#include "CppUTestExt/MockSupport.h"

#include <iostream>

#include "thermistor_sensor_service.hpp"
#include "mock_sensor.hpp"
#include "fake_config_store.hpp"
#include "fake_logger.hpp"
#include "fake_rtc.hpp"
#include "fake_timer.hpp"
#include "scheduler.hpp"


extern Timer *system_timer;
extern ConfigurationStore *configuration_store;
extern Scheduler *system_scheduler;
extern RTC *rtc;


TEST_GROUP(ThermistorSensor) {
	FakeConfigurationStore *fake_config_store;
	FakeTimer *fake_timer;
	FakeLog *fake_logger;
	Logger *logger;
	FakeRTC *fake_rtc;

	void setup() {
		fake_timer = new FakeTimer;
		system_timer = fake_timer;
		fake_rtc = new FakeRTC;
		rtc = fake_rtc;
		fake_config_store = new FakeConfigurationStore;
		configuration_store = fake_config_store;
		fake_logger = new FakeLog;
		logger = fake_logger;
		fake_logger->create();
		system_scheduler = new Scheduler(system_timer);
		configuration_store->init();
	}

	void teardown() {
		delete system_scheduler;
		delete fake_timer;
		delete fake_config_store;
		delete fake_logger;
		delete fake_rtc;
		// Reset globals so the next test group cannot dereference dangling pointers.
		system_scheduler = nullptr;
		system_timer = nullptr;
		rtc = nullptr;
		configuration_store = nullptr;
	}

	void notify_gnss_active() {
		ServiceEvent e;
		e.event_type = ServiceEventType::SERVICE_ACTIVE, e.event_source = ServiceIdentifier::GNSS_SENSOR;
		e.event_originator_unique_id = 0x12345678;
		ServiceManager::notify_peer_event(e);
	}

	void notify_gnss_inactive() {
		ServiceEvent e;
		e.event_type = ServiceEventType::SERVICE_INACTIVE, e.event_source = ServiceIdentifier::GNSS_SENSOR;
		e.event_originator_unique_id = 0x12345678;
		ServiceManager::notify_peer_event(e);
	}
};


TEST(ThermistorSensor, SensorDisabled) {
	MockSensor drv;
	ThermistorSensorService s(drv, logger);
	unsigned int num_callbacks = 0;

	system_timer->start();

	unsigned int period = 10;
	bool sensor_en = false;

	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE, sensor_en);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_PERIODIC, period);

	s.start([&num_callbacks](ServiceEvent &event) {
		if (event.event_type == ServiceEventType::SERVICE_LOG_UPDATED) {
			num_callbacks++;
		}
	});

	for (unsigned int i = 0; i < 5; i++) {
		fake_timer->increment_counter(period * 1000);
		system_scheduler->run();
	}

	CHECK_EQUAL(0, num_callbacks);
	CHECK_EQUAL(0, logger->num_entries());

	s.stop();
}

TEST(ThermistorSensor, SchedulingPeriodic) {
	MockSensor drv;
	ThermistorSensorService s(drv, logger);
	unsigned int num_callbacks = 0;

	system_timer->start();

	unsigned int period = 10;
	bool sensor_en = true;

	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE, sensor_en);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_PERIODIC, period);

	s.start([&num_callbacks](ServiceEvent &event) {
		if (event.event_type == ServiceEventType::SERVICE_LOG_UPDATED) {
			num_callbacks++;
		}
	});

	for (unsigned int i = 0; i < 5; i++) {
		mock().expectOneCall("read").onObject(&drv).withUnsignedIntParameter("port", 0).andReturnValue(
		    (double)(20.0 + i));
		fake_timer->increment_counter(period * 1000);
		system_scheduler->run();
	}

	CHECK_EQUAL(5, num_callbacks);
	CHECK_EQUAL(5, logger->num_entries());

	for (unsigned int i = 0; i < 5; i++) {
		ThermistorLogEntry e;
		logger->read(&e, i);
		DOUBLES_EQUAL(20.0 + i, e.temp, 0.001);
	}

	s.stop();
}

TEST(ThermistorSensor, SchedulingNoPeriodic) {
	MockSensor drv;
	ThermistorSensorService s(drv, logger);
	unsigned int num_callbacks = 0;

	system_timer->start();

	unsigned int period = 0;
	bool sensor_en = true;

	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE, sensor_en);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_PERIODIC, period);

	s.start([&num_callbacks](ServiceEvent &event) {
		if (event.event_type == ServiceEventType::SERVICE_LOG_UPDATED) {
			num_callbacks++;
		}
	});

	for (unsigned int i = 0; i < 5; i++) {
		fake_timer->increment_counter(10000);
		system_scheduler->run();
	}

	CHECK_EQUAL(0, num_callbacks);
	CHECK_EQUAL(0, logger->num_entries());

	s.stop();
}

TEST(ThermistorSensor, SchedulingTxEnableOneShot) {
	MockSensor drv;
	ThermistorSensorService s(drv, logger);
	unsigned int num_callbacks = 0;

	system_timer->start();

	unsigned int period = 10;
	bool sensor_en = true;
	BaseSensorEnableTxMode mode = BaseSensorEnableTxMode::ONESHOT;

	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE, sensor_en);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_PERIODIC, period);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE_TX_MODE, mode);

	s.start([&num_callbacks](ServiceEvent &event) {
		if (event.event_type == ServiceEventType::SERVICE_LOG_UPDATED) {
			num_callbacks++;
		}
	});

	notify_gnss_active();

	for (unsigned int i = 0; i < 1; i++) {
		mock().expectOneCall("read").onObject(&drv).withUnsignedIntParameter("port", 0).andReturnValue((double)25.0);
		fake_timer->increment_counter(period * 1000);
		system_scheduler->run();
	}

	notify_gnss_inactive();

	CHECK_EQUAL(1, num_callbacks);
	CHECK_EQUAL(1, logger->num_entries());

	ThermistorLogEntry e;
	logger->read(&e, 0);
	DOUBLES_EQUAL(25.0, e.temp, 0.001);

	s.stop();
}

TEST(ThermistorSensor, SchedulingTxEnableMean) {
	MockSensor drv;
	ThermistorSensorService s(drv, logger);
	unsigned int num_callbacks = 0;

	system_timer->start();

	unsigned int tx_period = 1;
	unsigned int period = 10;
	unsigned int max_samples = 100;
	bool sensor_en = true;
	BaseSensorEnableTxMode mode = BaseSensorEnableTxMode::MEAN;
	ServiceSensorData sensorData;

	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE, sensor_en);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_PERIODIC, period);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE_TX_MODE, mode);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE_TX_SAMPLE_PERIOD, tx_period);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE_TX_MAX_SAMPLES, max_samples);

	s.start([&num_callbacks, &sensorData](ServiceEvent &event) {
		if (event.event_type == ServiceEventType::SERVICE_LOG_UPDATED) {
			num_callbacks++;
			sensorData = std::get<ServiceSensorData>(event.event_data);
		}
	});

	notify_gnss_active();

	for (unsigned int i = 0; i < max_samples; i++) {
		mock().expectOneCall("read").onObject(&drv).withUnsignedIntParameter("port", 0).andReturnValue((double)i);
		fake_timer->increment_counter(period);
		system_scheduler->run();
	}

	notify_gnss_inactive();

	CHECK_EQUAL(1, num_callbacks);
	CHECK_EQUAL(1, logger->num_entries());
	ThermistorLogEntry e;
	logger->read(&e, 0);
	DOUBLES_EQUAL(49.5, e.temp, 0.001);
	DOUBLES_EQUAL(49.5, sensorData.port[0], 0.001);

	s.stop();
}

TEST(ThermistorSensor, SchedulingTxEnableMedian) {
	MockSensor drv;
	ThermistorSensorService s(drv, logger);
	unsigned int num_callbacks = 0;

	system_timer->start();

	unsigned int tx_period = 1;
	unsigned int period = 10;
	unsigned int max_samples = 100;
	bool sensor_en = true;
	BaseSensorEnableTxMode mode = BaseSensorEnableTxMode::MEDIAN;
	ServiceSensorData sensorData;

	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE, sensor_en);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_PERIODIC, period);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE_TX_MODE, mode);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE_TX_SAMPLE_PERIOD, tx_period);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE_TX_MAX_SAMPLES, max_samples);

	s.start([&num_callbacks, &sensorData](ServiceEvent &event) {
		if (event.event_type == ServiceEventType::SERVICE_LOG_UPDATED) {
			num_callbacks++;
			sensorData = std::get<ServiceSensorData>(event.event_data);
		}
	});

	notify_gnss_active();

	for (unsigned int i = 0; i < max_samples; i++) {
		mock().expectOneCall("read").onObject(&drv).withUnsignedIntParameter("port", 0).andReturnValue((double)i);
		fake_timer->increment_counter(period);
		system_scheduler->run();
	}

	notify_gnss_inactive();

	CHECK_EQUAL(1, num_callbacks);
	CHECK_EQUAL(1, logger->num_entries());
	ThermistorLogEntry e;
	logger->read(&e, 0);
	DOUBLES_EQUAL(49.5, e.temp, 0.001);
	DOUBLES_EQUAL(49.5, sensorData.port[0], 0.001);

	s.stop();
}

TEST(ThermistorSensor, WakeupThresholdTriggersGNSS) {
	MockSensor drv;
	ThermistorSensorService s(drv, logger);
	unsigned int num_callbacks = 0;
	unsigned int gnss_on_count = 0;

	system_timer->start();

	unsigned int period = 10;
	bool sensor_en = true;
	double threshold = 30.0;
	unsigned int wakeup_samples = 3;

	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE, sensor_en);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_PERIODIC, period);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_WAKEUP_THRESH, threshold);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_WAKEUP_SAMPLES, wakeup_samples);

	s.start([&num_callbacks, &gnss_on_count](ServiceEvent &event) {
		if (event.event_type == ServiceEventType::SERVICE_LOG_UPDATED) {
			num_callbacks++;
		}
		if (event.event_type == ServiceEventType::GNSS_ON) {
			gnss_on_count++;
		}
	});

	// Send values below threshold - no GNSS trigger
	for (unsigned int i = 0; i < 2; i++) {
		mock().expectOneCall("read").onObject(&drv).withUnsignedIntParameter("port", 0).andReturnValue((double)20.0);
		fake_timer->increment_counter(period * 1000);
		system_scheduler->run();
	}

	// Send values above threshold - should trigger after wakeup_samples consecutive
	for (unsigned int i = 0; i < wakeup_samples; i++) {
		mock().expectOneCall("read").onObject(&drv).withUnsignedIntParameter("port", 0).andReturnValue((double)35.0);
		fake_timer->increment_counter(period * 1000);
		system_scheduler->run();
	}

	CHECK_EQUAL(2 + wakeup_samples, num_callbacks);

	s.stop();
}

TEST(ThermistorSensor, WakeupThresholdDisabledWhenZeroSamples) {
	MockSensor drv;
	ThermistorSensorService s(drv, logger);
	unsigned int num_callbacks = 0;

	system_timer->start();

	unsigned int period = 10;
	bool sensor_en = true;
	double threshold = 30.0;
	unsigned int wakeup_samples = 0;  // Disabled

	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE, sensor_en);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_PERIODIC, period);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_WAKEUP_THRESH, threshold);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_WAKEUP_SAMPLES, wakeup_samples);

	s.start([&num_callbacks](ServiceEvent &event) {
		if (event.event_type == ServiceEventType::SERVICE_LOG_UPDATED) {
			num_callbacks++;
		}
	});

	// Send values above threshold - should NOT trigger GNSS because wakeup_samples = 0
	for (unsigned int i = 0; i < 5; i++) {
		mock().expectOneCall("read").onObject(&drv).withUnsignedIntParameter("port", 0).andReturnValue((double)35.0);
		fake_timer->increment_counter(period * 1000);
		system_scheduler->run();
	}

	CHECK_EQUAL(5, num_callbacks);
	CHECK_EQUAL(5, logger->num_entries());

	s.stop();
}

TEST(ThermistorSensor, IsUsableUnderwater) {
	MockSensor drv;
	ThermistorSensorService s(drv, logger);

	system_timer->start();

	bool sensor_en = true;
	unsigned int period = 10;
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_ENABLE, sensor_en);
	configuration_store->write_param(ParamID::THERMISTOR_SENSOR_PERIODIC, period);

	s.start([](ServiceEvent &) {});

	// Thermistor should be usable underwater
	mock().expectOneCall("read").onObject(&drv).withUnsignedIntParameter("port", 0).andReturnValue((double)22.0);
	fake_timer->increment_counter(period * 1000);
	system_scheduler->run();

	CHECK_EQUAL(1, logger->num_entries());

	s.stop();
}
