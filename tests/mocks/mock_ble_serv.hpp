...
public:
	virtual bool isEqual(const void *object1, const void *object2) {
		...
		if (object1_func_ptr->target_type() != object2_func_ptr->target_type()) return false;
		...
		if (object1_func_target != object2_func_target) return false;
...
public:
	void start(std::function<int(BLEServiceEvent &)> on_event) {
		...
		mock().actualCall("start").onObject(this).withParameterOfType("std::function<int(BLEServiceEvent&)>",
		                                                              "on_event", &on_event);
		m_on_event = on_event;
	}
	...
	bool write(std::string str) {
		return mock()
		    .actualCall("write")
		    .onObject(this)
		    .withParameterOfType("std::string", "str", &str)
		    .returnBoolValue();
	}
	std::string read_line() {
		// Wrap dereference in null check to satisfy g++13 -Werror=null-dereference.
		const std::string *ptr =
		    static_cast<const std::string *>(mock().actualCall("read_line").onObject(this).returnConstPointerValue());
	...
	int invoke_event(BLEServiceEvent &event) { return m_on_event(event); }
