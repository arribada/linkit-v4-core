...
public:
	void start_file_transfer(OTAFileIdentifier file_id, const lfs_size_t length, const uint32_t crc32) override {
		mock()
		    .actualCall("start_file_transfer")
		    .onObject(this)
		    .withParameter("file_id", (unsigned int)file_id)
		    .withParameter("length", length)
		    .withParameter("crc32", crc32);
	}
	...
	void abort_file_transfer() override { mock().actualCall("abort_file_transfer").onObject(this); }
	void complete_file_transfer() override { mock().actualCall("complete_file_transfer").onObject(this); }
	void apply_file_update() override { mock().actualCall("apply_file_update").onObject(this); }
};
