...
nrfx_err_t nrfx_rtc_init(nrfx_rtc_t const *const p_instance, nrfx_rtc_config_t const *p_config,
	...
	if (m_cb[p_instance->instance_id].state != NRFX_DRV_STATE_UNINITIALIZED) return NRFX_ERROR_INVALID_STATE;
...
void nrfx_rtc_uninit(nrfx_rtc_t const *const p_instance) {
	m_cb[p_instance->instance_id].handle = NULL;
	m_cb[p_instance->instance_id].counter = 0;
	m_cb[p_instance->instance_id].overflow_irq = false;
...
void nrfx_rtc_enable(nrfx_rtc_t const *const p_instance) {
	if (m_cb[p_instance->instance_id].state != NRFX_DRV_STATE_INITIALIZED) return;

	m_cb[p_instance->instance_id].state = NRFX_DRV_STATE_POWERED_ON;
}

void nrfx_rtc_disable(nrfx_rtc_t const *const p_instance) {
	if (m_cb[p_instance->instance_id].state == NRFX_DRV_STATE_UNINITIALIZED) return;
...
nrfx_err_t nrfx_rtc_cc_disable(nrfx_rtc_t const *const p_instance, uint32_t channel) {
	...
	m_cb[p_instance->instance_id].compare_channel[channel].irq_enabled = false;
...
void nrfx_rtc_increment_tick(void) {
	for (uint32_t i = 0; i < NRFX_RTC_ENABLED_COUNT; ++i) {
		if (NRFX_DRV_STATE_POWERED_ON == m_cb[i].state) {
			...
			if (m_cb[i].counter == 0 && m_cb[i].overflow_irq) {
				if (m_cb[i].handle) m_cb[i].handle(NRFX_RTC_INT_OVERFLOW);
			}

			// Check for compare channel conditions
			for (uint32_t j = 0; j < NUMBER_COMPARE_CHANNELS; ++j) {
				if (m_cb[i].compare_channel[j].value == m_cb[i].counter && m_cb[i].compare_channel[j].irq_enabled) {
					m_cb[i].compare_channel[j].irq_enabled = false;
					if (m_cb[i].handle) m_cb[i].handle(j);
				}
			}
		}
	}
