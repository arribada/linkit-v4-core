...
void SWSAnalogService::validate_diagnostics() {
	...
	if (calc != m_diag_crc) {
		...
		DEBUG_INFO("SWSAnalog: Diagnostics restored from noinit "
		           ...
		           m_diag.stuck_recovery_count, m_diag.coherence_recalib_count, m_diag.dive_timeout_count,
		           m_diag.force_surface_count, m_diag.spike_reject_count, m_diag.peak_incoherent_count,
...
void SWSAnalogService::cancel_guided_calibration() {
	if (m_calib_phase == CalibPhase::IDLE || m_calib_phase == CalibPhase::DONE) return;
...
bool SWSAnalogService::is_guided_calibration_running() {
	return m_calib_phase != CalibPhase::IDLE && m_calib_phase != CalibPhase::DONE;
}
...
void SWSAnalogService::service_init() {
	...
	if (delay_crc == m_sample_delay_us_crc && m_sample_delay_us_noinit >= m_delay_min_us
	...
	uint16_t peak_crc = crc16_compute((const uint8_t *)&m_observed_peak_adc, sizeof(m_observed_peak_adc), nullptr);
	...
	if (!validate_calibration_data()) {
		...
		DEBUG_INFO("SWSAnalog: Calibration valid - air=%u water=%u thresh=%u", m_calib.threshold_air,
	...
	DEBUG_INFO("SWSAnalog: Init - hyst=%u%% ratio=%u%%", m_hysteresis_percent, m_threshold_ratio_percent);
}
...
unsigned int SWSAnalogService::service_next_schedule_in_ms() {
	...
	double period_s = m_current_state ? service_read_param<double>(ParamID::SAMPLING_UNDER_FREQ)
...
uint16_t SWSAnalogService::read_analog_sws() {
	...
	nrf_gpio_cfg(sws_pin, NRF_GPIO_PIN_DIR_INPUT, NRF_GPIO_PIN_INPUT_DISCONNECT, NRF_GPIO_PIN_NOPULL, NRF_GPIO_PIN_S0S1,
