...
private:
	...
	static int lfs_read(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size) {
		return reinterpret_cast<LFSFileSystem *>(c->context)->m_flash_if->read(block, off, buffer, size);
	}
	static int lfs_prog(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, const void *buffer,
	                    lfs_size_t size) {
		return reinterpret_cast<LFSFileSystem *>(c->context)->m_flash_if->prog(block, off, buffer, size);
	}
	static int lfs_erase(const struct lfs_config *c, lfs_block_t block) {
		return reinterpret_cast<LFSFileSystem *>(c->context)->m_flash_if->erase(block);
	}
	static int lfs_sync(const struct lfs_config *c) {
		return reinterpret_cast<LFSFileSystem *>(c->context)->m_flash_if->sync();
	}
...
public:
	...
	LFSFileSystem(FlashInterface *flash_if, unsigned int blocks = 0) {
		...
		if (0 == blocks) blocks = flash_if->m_blocks;
	...
	bool is_mounted() override { return m_is_mounted; }
	...
	int remove(const char *path) override { return lfs_remove(&m_lfs, path); }

	lfs_size_t get_block_size() { return m_cfg.block_size; }

	int stat(const char *path, struct lfs_info *info) override { return lfs_stat(&m_lfs, path, info); }
	...
	void *get_private_data() override { return &m_lfs; }
...
public:
	RamFlash(unsigned int block_size, unsigned int blocks, unsigned int page_size)
	    : FlashInterface(block_size, blocks, page_size) {
		m_block_ram = new uint8_t[block_size * blocks];
		m_debug_trace = false;
	}
	~RamFlash() { delete[] m_block_ram; }

	int read(lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size) override {
		if (m_debug_trace) printf("read(%lu %lu %lu)\n", block, off, size);
...
public:
	...
	virtual ~LFSFile() { lfs_file_close(m_lfs, &m_file); }
	lfs_ssize_t read(void *buffer, lfs_size_t size) override { return lfs_file_read(m_lfs, &m_file, buffer, size); }
	lfs_ssize_t write(void *buffer, lfs_size_t size) override { return lfs_file_write(m_lfs, &m_file, buffer, size); }
	lfs_soff_t seek(lfs_soff_t off, int whence = LFS_SEEK_SET) override {
		return lfs_file_seek(m_lfs, &m_file, off, whence);
	}
	lfs_soff_t tell() override { return lfs_file_tell(m_lfs, &m_file); }
	int flush() override { return lfs_file_sync(m_lfs, &m_file); }
	lfs_soff_t size() override { return lfs_file_size(m_lfs, &m_file); }
};
...
public:
	...
	LFSCircularFile(FileSystem *fs, const char *path, int flags, lfs_size_t max_size) : LFSFile(fs, path, flags) {
		...
		if (file_size < 0) throw file_size;

		// Ensure we seek to the oldest entry in the file when file hasn't yet wrapped
		if (flags & LFS_O_RDONLY && static_cast<lfs_size_t>(file_size) < max_size) m_offset = 0;

		seek(m_offset);
	}

	~LFSCircularFile() {
		if (m_flags & LFS_O_WRONLY) lfs_setattr(m_lfs, m_path, 0, &m_offset, sizeof(m_offset));
	}

	lfs_off_t get_offset() { return m_offset; }
	...
	lfs_soff_t tell() { return static_cast<lfs_soff_t>(m_offset); }
};
